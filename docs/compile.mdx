...
export const UploadTab = ({onDataReady}) => {
  ...
  return (
    <div>
      <p>➜ Click an image of any hardware object to be targetted, make sure it has a good contrast.</p>
      <p>➜ Make sure to crop your selected images in order to keep the object in focus.</p>
      <p>➜ Drop target images and hit start.</p>
      <p>➜ Finally after your image target gets compiled, tap next and start making annotations in real time.</p>
      <p>➜ Make sure that you hold your phone parallel to the image target for a better accuracy.</p>
...
export const VisualizeTab = ({dataList, exportedBuffer}) => {
  ...
  const sendHw = async(height, width) =>{
  	 let requestOptions = {method: 'GET',redirect: 'follow'};
	 let response  = await fetch(`https://artest.supportgenie.io/hw?height=${height}&width=${width}`, requestOptions);
  }
  useEffect(async() => {
    ...
    await sendHw(targetImage.height, targetImage.width);
    ...
    for (let i = 0; i < matchingPoints.length; i++) {
      ...
      drawPoint(ctx, '#E45454', Math.round(point.x), Math.round(point.y), point.scale);
    }
  ...
  const saveMind = async(blob) => {
    ...
    const response = await fetch("https://artest.supportgenie.io/upload", requestOptions);
  ...
  const downloadHandler = useCallback(async() => {
    ...
    aLink.href = "http://localhost:3000/AR/index.html";
    aLink.click();
  }, [exportedBuffer]);
  return (
    <div>
	...
	<canvas style={canvasStyle} className={styles.visualizeCanvas} ref={canvasRef}></canvas>
      ...
      <div className="padding-vert--md">
	<button className={clsx("button", "button--primary", styles.startButton)} onClick={downloadHandler}>Tap Next & Wait</button>
      </div>
    </div>
  )
}
