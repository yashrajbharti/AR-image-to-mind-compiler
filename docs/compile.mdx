...
export const VisualizeTab = ({dataList, exportedBuffer}) => {
  ...
  const sendHw = async(height, width) =>{
	 ...
	 let response  = await fetch(`http://192.241.202.10:80/hw?height=${height}&width=${width}`, requestOptions);
  }
  useEffect(async() => {
  ...
  const saveMind = async(blob) => {
    ...
    const response = await fetch("http://192.241.202.10:80/upload", requestOptions);
